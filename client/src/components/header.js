...
import NavDropdownItem from 'react-bootstrap/NavItem'
...
class Header extends React.Component {
    ...
    renderTemplate() {
        if (this.props.user.id) {
            const loggedUserId = global.auth.getUserId().id;
            const mainScreenLink = '/'+ loggedUserId + '/templatemain';
            const mainContactLink = '/'+ loggedUserId + '/temcontact';
            const mainAchivementLink = '/'+ loggedUserId + '/temachivement';
            const mainExperienceLink = '/'+ loggedUserId + '/temexperience';
            return (
                <React.Fragment>
                    <NavDropdown title="Template" style={{marginTop:'8px'}}>
                        <NavDropdownItem>
                            <Link className="navbar-item" to={mainScreenLink}>
                                MainScreen
                            </Link>
                        </NavDropdownItem>
                        <NavDropdownItem>
                            <Link className="navbar-item" to={mainAchivementLink}>
                                Achivement
                            </Link>
                        </NavDropdownItem>
                        <NavDropdownItem>
                            <Link className="navbar-item" to={mainExperienceLink}>
                                Experience
                            </Link>
                        </NavDropdownItem>
                        <NavDropdownItem>
                            <Link className="navbar-item" to={mainContactLink}>
                                Contact
                            </Link>
                        </NavDropdownItem>
                    </NavDropdown>
